...
the northern sinking region. The overtruning circulation is computed at
...
# create S.O. surface boundary conditions and grid
y=np.asarray(np.linspace(0,2e6, 40));
tau=0.17*np.sin(np.pi*y/2e6);
...
total_iters=int(np.ceil(2000*360*86400/dt))  # total number of timesteps
...
SO=Model_SO(z=z,y=y,b=b_basin(z),bs=bs_SO,tau=tau,L=5e6,KGM=1000)
SO.solve()
...
for ii in range(0, total_iters):    
   ...
   wb=(AMOC.Psi_N-SO.Psi)*1e6/A_basin
   ...
   if ii%plot_iters==0:
      # Plot current state:
      ax1.plot(AMOC.Psi_N, AMOC.z, linewidth=0.5, color='r')
